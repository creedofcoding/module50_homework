...
class Route
{
    public static function start()
    {
        ...
        if (in_array($routes, ['register', 'login', 'logout'])) {
            $controller_name = 'Controller_user';
            $action_name = 'action_' . $routes; // Преобразуем в нужное действие
            $model_name = 'model_user';
        } else if (in_array($routes, ['gallery', 'upload_images', 'view_image', 'delete_image'])) { 
            $controller_name = 'Controller_gallery';
            $action_name = 'action_' . $routes; // Преобразуем в нужное действие
            $model_name = 'model_gallery';
        } else if (in_array($routes, ['add_comment', 'delete_comment'])) {
            $controller_name = 'Controller_comment';
            $action_name = 'action_' . $routes; // Преобразуем в нужное действие
